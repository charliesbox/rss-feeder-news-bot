...
async def titles_kb(callback: CallbackQuery):
    ...
    if page_offset < len(titles):
        builder.button(text='следующая страница', callback_data=f'titles_{agency}_{number}_{(page_offset // 5) + 1}')    #if you're customizing the amount of news
    if page_offset - 5 > 0:                                                                                              #in one page, make sure to change the number here too
